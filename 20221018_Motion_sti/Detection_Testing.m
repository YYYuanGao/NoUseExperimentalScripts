function Detection_Testing(SubjID,session,block,MotionDirection)

% subj_id = 'jiake'
% by Ke Jia
% Last modified 2021/10/28 09:45
...
task_dif = Param.Staircase.Coherence;
...
while (~UD.stop & (size(UD.response)<Param.Staircase.MaxTrial)) 
    ...
    results(trial_i,2) = Param.DetectTest.StiLocation;           %2刺激位置
...
%数据存储
CurrDir = pwd;
resultsDir = [CurrDir '\Results\DetectTest\' SubjID '\'];
if ~isdir(resultsDir)                                                       %isder和mkder可以用来创建新文件夹
    mkdir(resultsDir);
end

cd(resultsDir);
results_name = [SubjID '_DetectTest_results_session' num2str(session) '_block' num2str(block) '.mat'];
