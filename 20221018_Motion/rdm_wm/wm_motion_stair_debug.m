function wm_motion_stair_debug(SubjID,session,block,MotionCoh,InitDiff)
...
%                           %21：reaction time
%                           %22：trial duration
%                           %23：initial difference threshold
...
for trial_i = 1:Param.DisPrac.TrialNum
    ...
    results(trial_i,3) = Param.DisPrac.Directions(Curr_cond);   %3- 1st baseline motion direction
    Curr_cond2 =  mod(Curr_cond+randi(5),6)+1;
    while Curr_cond2 == Curr_cond
        Curr_cond2 =  mod(Curr_cond+randi(5),6)+1;
    end
    results(trial_i,24) =Param.DisPrac.Directions(Curr_cond2);%24- 2nd baseline motion direction
...
%       results(trial_i,5) = results(trial_i,4) + trial_delta_ori * task_dif;
        results(trial_i,4) = results(trial_i,3) + (rand-0.5)*2*jitter;
        
        results(trial_i,5) = results(trial_i,24)+ (rand-0.5)*2*jitter;
...
%     test_no = ((rand-0.5)>=0)+1;
    ...
    % 8-baseline for test, need to add average difference
    results(trial_i,25) = results(trial_i,8)+stairCurr;

    % test sample
    MyDot_t = Gen_DotMatrix_bm(results(trial_i,25),results(trial_i,2),Param.RDK.TestNumFrames,Param.RDK.DotNum,Param.RDK.InnerRadius,Param.RDK.OuterRadius,Param.RDK.StepPerMove);
...
%         Screen('DrawDots',wnd,MyDot_t{1,frame_i},Param.RDK.DotSize,black,Param.Stimuli.Locations(1,:),1);
    ...
    while (is_true == 0 && GetSecs - results(trial_i,20) < Param.Trial.MaxRT)
        ...
        if keyCode(Param.Keys.Right) || keyCode(Param.Keys.two1) || keyCode(Param.Keys.two2)
            ...
            if results(trial_i,25) ~= results(trial_i,8)
    ...
    if results(trial_i,10) || ~stairFlag                      % correct trial
        stairCurr = stairCurr-stairDerta;
        stairStep = stairStep+1;
        if stairStep == 3
            stairFlag = 1;
            stairStep = 0;
            stairChange = stairChange+1;
        end
    elseif ~results(trial_i,10) || stairFlag
        beep;
        stairCurr = stairCurr+stairDerta;
        stairStep = stairStep+1;
        if stairStep == 3
