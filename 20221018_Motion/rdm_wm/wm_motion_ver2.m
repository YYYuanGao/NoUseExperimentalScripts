function wm_motion_ver2(SubjID,session,block,MotionCoh)
...
%3：base direction       %12：生成运动刺激需要时间,注意这个值要小于600ms
%4：sample1 direction    %13：运动刺激1呈现时间点
%5：sample2 direction(base)    %14：刺激间隔1呈现时间点
%6：cue                  %15：运动刺激2呈现时间点
...
%         Screen('DrawDots',wnd,MyDot_t{1,frame_i},Param.RDK.DotSize,black,Param.Stimuli.Locations(1,:),1);
    ...
    while (is_true == 0 && GetSecs - results(trial_i,19) < Param.Trial.MaxRT)
        ...
        if keyCode(Param.Keys.Right) || keyCode(Param.Keys.two1) || keyCode(Param.Keys.two2)
            results(trial_i,7) = 2;              % subj responds the test sample is not corresponding to cue
            if sample_no ~= cue_no
                results(trial_i,8) = 1;          % Correct: test is different to cue
            end
        ...
        elseif keyCode(Param.Keys.Left) || keyCode(Param.Keys.one1) || keyCode(Param.Keys.one2)
            results(trial_i,7) = 1;              % subj responds the test sample is corresponding to cue
            if sample_no == cue_no
                results(trial_i,8) = 1;
            end
