function DotMatrix = Gen_DotMatrix_bm(MotionDir,MotionCoh,NumFrames,DotNum,InnerRadius,OuterRadius,StepPerMove)
    ...
    for frame_i = 2:NumFrames 
        ...
        for dot_i = 1:DotNum 
            ...
            if ismember(dot_i,SignalDots)              
                DotMatrix{1,frame_i}(:,dot_i) = DotMatrix{1,frame_i-1}(:,dot_i) + Curr_Move;
            else
                temp_angle = rand()*2*pi;
                temp_move = [StepPerMove*cos(temp_angle);StepPerMove*sin(temp_angle)];
                DotMatrix{1,frame_i}(:,dot_i) = DotMatrix{1,frame_i-1}(:,dot_i) + temp_move;
            end  
