...
if exist([CurrDir '\Results\practise\' SubjID '\' SubjID '_results_sess' num2str(sess_num) '_run' num2str(run_num) '.mat'],'file')
    disp(' ');
...
%% 
results = zeros(Param.Trial.Practice,22);
    ...
    while (is_true == 0)
        ...
        elseif keyCode(Param.Keys.Space)
            ...
            if results(trial_i,22) == results(trial_i,2)
                results(trial_i,19) = CalAngleDiff90(abs(results(trial_i,12)-results(trial_i,8)));
            elseif results(trial_i,22) == results(trial_i,3)
                results(trial_i,19) = CalAngleDiff90(abs(results(trial_i,12)-results(trial_i,9)));
            elseif results(trial_i,22) == results(trial_i,4)
                results(trial_i,19) = CalAngleDiff90(abs(results(trial_i,12)-results(trial_i,10)));
            end

            error_ori = results(trial_i,19);
            DrawFormattedText(wnd,double(['error ori now : ' num2str(error_ori)]),'center','center',black);
            Screen('Flip',wnd);
            WaitSecs(1.5);
        elseif keyCode(Param.Keys.Esc)
            abort;
...
%% show mean error
mean_error = mean(results(:,19));
DrawFormattedText(wnd,double(['mean error : ' num2str(mean_error)]),'center','center',black);
Screen('Flip',wnd);
WaitSecs(2);

%% save the data
resultsDir = [CurrDir '\Results\practice\' SubjID '\'];
