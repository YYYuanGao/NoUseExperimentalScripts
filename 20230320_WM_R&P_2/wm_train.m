...
if exist([CurrDir '\Results\train\' SubjID '\' SubjID '_results_sess' num2str(sess_num) '_run' num2str(run_num) '.mat'],'file')
    disp(' ');
...
%% 
results = zeros(Param.Trial.TrainNum,23);
trial_index = randperm(Param.Trial.TrainNum);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% results:
...
% 5 ori for reward          % 14 fix onset delay            % 23 points
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Go!
    for trial_i = 1:Param.Trial.TrainNum
    % start
    if mod(trial_i,Param.Trial.Trainminirun)==1
        if trial_i == 1
            DrawFormattedText(wnd,'Press space to start!','center','center', black);
        else
            DrawFormattedText(wnd,double(['Take a rest! Press space to continue! \n\n Total points now:' num2str(total_points)]),'center','center',black);
        end
    ...
    while (is_true == 0)
        ...
        elseif keyCode(Param.Keys.Space)
            ...
            if results(trial_i,22) == results(trial_i,2)
                if results(trial_i,19) < 3 || results(trial_i,19) > -3
                    DrawFormattedText(wnd,'+4 points','center','center', black);
                    results(trial_i,23) = 4;
                elseif results(trial_i,19) < 5 || results(trial_i,19) > -5
                    DrawFormattedText(wnd,'+3 points','center','center', black);
                    results(trial_i,23) = 3;
                elseif results(trial_i,19) < 7 || results(trial_i,19) > -7
                    DrawFormattedText(wnd,'+2 points','center','center', black);
                    results(trial_i,23) = 2;
                elseif results(trial_i,19) > 7 || results(trial_i,19) < -7
                    DrawFormattedText(wnd,'+1 point','center','center', black);
                    results(trial_i,23) = 1;
                end
            elseif results(trial_i,22) == results(trial_i,3)
                if results(trial_i,19) < 3 || results(trial_i,19) > -3
                    DrawFormattedText(wnd,'-1 point','center','center', black);
            ...
            elseif results(trial_i,22) == results(trial_i,4)
                DrawFormattedText(wnd,'0 point','center','center', black);
                results(trial_i,23) = 0;
            end
            Screen('Flip',wnd);
            WaitSecs(1.5);
        elseif keyCode(Param.Keys.Esc)
    ...
    total_points = sum(results(:,23));
    end

    %% show total points
    DrawFormattedText(wnd,double(['Total points now:' num2str(total_points)]),'center','center',black);
    Screen('Flip',wnd);
    WaitSecs(2);

    %% save the data
    resultsDir = [CurrDir '\Results\train\' SubjID '\'];
